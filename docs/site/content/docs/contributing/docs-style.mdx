...
order. Examples: `how-to/add-ddd-layers`, `how-to/use-hatchling`,
`tutorials/your-first-command`.
...
are available but not yet used by any page here. Reach for them when
they add value (a `Callout` for a warning the reader must not miss;
`Cards` / `LinkCard` for landing-page navigation).
